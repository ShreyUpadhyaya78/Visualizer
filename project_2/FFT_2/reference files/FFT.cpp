...
void FFT::hammingWindow()
{
	...
	if(mark+bufferSize < sampleCount)
	{
		for(int i(mark) ; i < bufferSize+mark ; i++)
		{
			...
			VA1[i-mark] = Vertex(Vector2f(20,250)+Vector2f((i-mark)/(float)bufferSize*700,sample[i-mark].real()*0.005),sf::Color(255,0,0,50)) ;
		}
	}
}
...
void FFT::bars(float const& max)
{
	...
	for(float i(3) ; i < min(bufferSize/2.f,20000.f) ; i*=1.01)
	{
		...
		VA2.append(Vertex(position+Vector2f(samplePosition.x*800,-samplePosition.y/max*500),sf::Color::White)) ;
		VA2.append(Vertex(position+Vector2f(samplePosition.x*800,0),sf::Color::White)) ;
		VA2.append(Vertex(position+Vector2f(samplePosition.x*800,0),sf::Color(255,255,255,100))) ;
		VA2.append(Vertex(position+Vector2f(samplePosition.x*800,samplePosition.y/max*500/2.f),sf::Color(255,255,255,0))) ;
	}
}
void FFT::lines(float const& max)
{
	...
	for(float i(std::max((double)0,cascade.size()-3e5)) ; i < cascade.size() ; i++)
	{
		...
		if(cascade[i].color.a != 0) cascade[i].color = sf::Color(255,255,255,20) ;
	}
	samplePosition = Vector2f(log(3)/log(min(bufferSize/2.f,20000.f)),abs(bin[(int)3])) ;
	cascade.push_back(Vertex(position+Vector2f(samplePosition.x*800,-samplePosition.y/max*500),sf::Color::Transparent)) ;
	for(float i(3) ; i < bufferSize/2.f ; i*=1.02)
	{
		samplePosition = Vector2f(log(i)/log(min(bufferSize/2.f,20000.f)),abs(bin[(int)i])) ;
		cascade.push_back(Vertex(position+Vector2f(samplePosition.x*800,-samplePosition.y/max*500),sf::Color(255,255,255,20))) ;
	}
	cascade.push_back(Vertex(position+Vector2f(samplePosition.x*800,-samplePosition.y/max*500),sf::Color::Transparent)) ;
...
wave::wave(float y1){
    y=y1;
void wave::tw(){
for(int i=390;i<1366;i++){
    v[i]=sf::Vertex(sf::Vector2f(i,y),sf::Color::White);
}
